process clinvar_assignment {
    ...
    grep -E "CLNSIG=Pathogenic|CLNSIG=Likely_patho" clinvar.intersect.vcf | awk -F '\\t' '{print \$1, \$2-1, \$2}' | sed 's/ /\\t/g' > clinvar.intersect.patho.vcf
    grep -E -v "CLNSIG=Benign|CLNSIG=Likely_benign|CLNSIG=Pathogenic|CLNSIG=Likely_patho" clinvar.intersect.vcf | awk -F '\\t' '{if(!(\$0~/^#/)) print \$1, \$2-1, \$2}' | sed 's/ /\\t/g' > clinvar.intersect.uncertain.vcf
